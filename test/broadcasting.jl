...
@testset "broadcasting of AbstractDataFrame objects" begin
    for df in (copy(refdf), view(copy(refdf), :, :))
        ...
        @test (Matrix(df) .+ df .== 2 .* df) == WDF(DataFrame(trues(size(df)), names(df)))
        @test df .+ 1 == df .+ ones(size(df))
        @test df .+ axes(df, 1) == WDF(DataFrame(Matrix(df) .+ axes(df, 1), names(df)))
        @test df .+ permutedims(axes(df, 2)) == WDF(DataFrame(Matrix(df) .+ permutedims(axes(df, 2)), names(df)))
    end

    df1 = copy(refdf)
    df2 = view(copy(refdf), :, :)
    @test (df1 .+ df2) ./ 2 == refdf
    @test (df1 .- df2) == WDF(DataFrame(zeros(size(refdf))))
    @test (df1 .* df2) == refdf .^ 2
    @test (df1 ./ df2) == WDF(DataFrame(ones(size(refdf))))
end
...
@testset "broadcasting expansion" begin
    df1 = WDF(DataFrame(x=1, y=2))
    df2 = WDF(DataFrame(x=[1,11], y=[2,12]))
    @test df1 .+ df2 == WDF(DataFrame(x=[2,12], y=[4,14]))
    ...
    @test df2.x === x
    @test df2.y === y
    @test df2 == WDF(DataFrame(x=[2,12], y=[4,14]))

    df = WDF(DataFrame(x=[1,11], y=[2,12]))
    dfv = view(df, 1:1, 1:2)
    df .-= dfv
    @test df == WDF(DataFrame(x=[0,10], y=[0,10]))
end
...
@testset "broadcasting of AbstractDataFrame objects corner cases" begin
    ...
    df5 = WDF(DataFrame(x = Any[1, 2, 3], y = Any[1, 2.0, big(3)]))
    @test identity.(df5) == df5
    @test (x->x).(df5) == df5
    @test df5 .+ 1 == WDF(DataFrame(Matrix(df5) .+ 1, names(df5)))
...
@testset "extending data frame in broadcasted assignment - one column" begin
    ...
    df[!, :b] .= sin.(1)
    df[!, :c] .= sin(1) .+ 1
    @test df == WDF(DataFrame(b=Float64[], c=Float64[]))
...
@testset "tuple broadcasting" begin
    X = WDF(DataFrame(zeros(2, 3)))
    X .= (1, 2)
    @test X == WDF(DataFrame([1 1 1; 2 2 2]))

    X = WDF(DataFrame(zeros(2, 3)))
    X .= (1, 2) .+ 10 .- X
    @test X == WDF(DataFrame([11 11 11; 12 12 12]))
    ...
    df = WDF(DataFrame(rand(2, 3)))
    @test floor.(Int, df ./ (1,)) == WDF(DataFrame(zeros(Int, 2, 3)))
    df .= floor.(Int, df ./ (1,))
    @test df == WDF(DataFrame(zeros(2, 3)))

    df = WDF(DataFrame(rand(2, 3)))
    @test_throws InexactError convert.(Int, df)
    df2 = convert.(Int, floor.(df))
    @test df2 == WDF(DataFrame(zeros(Int, 2, 3)))
...
@testset "nothing test" begin
    X = WDF(DataFrame(Any[1 2; 3 4]))
    X .= nothing
    @test (X .== nothing) == WDF(DataFrame(trues(2, 2)))

    X = WDF(DataFrame([1 2; 3 4]))
    @test_throws MethodError X .= nothing
    @test X == WDF(DataFrame([1 2; 3 4]))
...
@testset "aliasing test" begin
    ...
    df = WDF(DataFrame(x=2, y=1, z=1))
    dfr = df[1, :]
    y = view(df.x, 1)
    dfr .= 2 .* y
    @test Vector(dfr) == [4, 4, 4]

    df = WDF(DataFrame(x=[1, 2], y=[11,12]))
    df2 = WDF(DataFrame())
    df2.x = [-1, -2]
    df2.y = df.x
    df3 = copy(df2)
    ...
    Random.seed!(1234)
    for i in 1:10
        df1 = WDF(DataFrame(rand(100, 100)))
        df2 = copy(df1)
        for i in 1:100
    ...
    for i in 1:10
        ...
        df6 = copy(df2)
        df7 = copy(df3)
        df4 = WDF(DataFrame(sin.(df1[1,1] .+ copy(df1[!, 1]) .+ Matrix(df2) ./ Matrix(df3))))
...
@testset "@. test" begin
    ...
    r1 = @. (df + sdf + 5) / sdf
    r2 = @. (df + sdf + 5) / sdf
    @test r1 == WDF(DataFrame(r2))

    @. df = sin(sdf / (df + 1))
    @. dfm = sin(sdfm / (dfm + 1))
    @test df == WDF(DataFrame(dfm))
end

@testset "test common cases" begin
    m = rand(1000, 10)
    df = WDF(DataFrame(m))
    @test df .+ 1 == WDF(DataFrame(m .+ 1))
    @test df .+ transpose(1:10) == WDF(DataFrame(m .+ transpose(1:10)))
    @test df .+ (1:1000) == WDF(DataFrame(m .+ (1:1000)))
    @test df .+ m == WDF(DataFrame(m .+ m))
    @test m .+ df == WDF(DataFrame(m .+ m))
    @test df .+ df == WDF(DataFrame(m .+ m))

    df .+= 1
    m .+= 1
    @test df == WDF(DataFrame(m))
    df .+= transpose(1:10)
    m .+= transpose(1:10)
    @test df == WDF(DataFrame(m))
    df .+= (1:1000)
...
@testset "DataFrameRow getproperty broadcasted assignment" begin
    df = WDF(DataFrame(a=[[1,2],[3,4]], b=[[5,6],[7,8]]))
    dfr = df[1, :]
    dfr.a .= 10
    @test df == WDF(DataFrame(a=[[10,10],[3,4]], b=[[5,6],[7,8]]))
...
@testset "broadcasting into df[!, cols]" begin
    for selector in [1:2, Between(:x1, :x2), Not(r"x3"), [:x1, :x2],
                     ["x1", "x2"], Between("x1", "x2")]
        df = WDF(DataFrame(x1=1:3, x2=4:6))
        df[!, selector] .= "a"
        @test df == WDF(DataFrame(fill("a", 3, 2)))
        @test df.x1 !== df.x2

        df = WDF(DataFrame(x1=1:3, x2=4:6))
        df[!, selector] .= Ref((a=1,b=2))
        @test df == WDF(DataFrame(fill((a=1, b=2), 3, 2)))
        @test df.x1 !== df.x2
        ...
        df = WDF(DataFrame(x1=1:3, x2=4:6, x3=1))
        df[!, selector] .= "a"
        ...
        df = WDF(DataFrame(x1=1:3, x2=4:6, x3=1))
        df[!, selector] .= Ref((a=1,b=2))
        @test df[:, 1:2] == WDF(DataFrame(fill((a=1, b=2), 3, 2)))
        @test df[:, 3] == [1, 1, 1]
        @test df.x1 !== df.x2
...
@testset "broadcasting over heterogenous columns" begin
    df = WDF(DataFrame(x = [1, 1.0, big(1), "1"]))
    f_identity(x) = x
    @test df == f_identity.(df)
end

@testset "@views on df[!, col]" begin
    df = WDF(DataFrame(ones(3, 4)))
    @views df[!, 1] .+= 1
